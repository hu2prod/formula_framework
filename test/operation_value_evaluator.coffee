...
describe 'operation_value_evaluator section', ()->
  ...
  describe 'bin_op', ()->
    describe 'add', ()->
      it '1 + 2 = 3', ()->
        assert val(3).value.weak_eq mod.eval op.add val(1), val(2)
        return
      
      it '1±0.3 + 2 = 3±0.3', ()->
        assert val(3, 0.3).value.weak_eq mod.eval op.add val(1, 0.3), val(2)
      
      it '1±0.3 + 2±0.4 = 3±0.7', ()->
        assert val(3, 0.7).value.weak_eq mod.eval op.add val(1, 0.3), val(2, 0.4)
      ...
      it '1[-0.1+0.2] + 2[-0.3+0.5] = 3[-0.4+0.7]', ()->
        assert val(3, 0.4, 0.7).value.weak_eq mod.eval op.add val(1, 0.1, 0.2), val(2, 0.3, 0.5)
    ...
    describe 'mul', ()->
      it '2 * 3 = 6', ()->
        assert val(6).value.weak_eq mod.eval op.mul val(2), val(3)
        return
      
      it '2±1 * 3 = 6±3', ()->
        assert val(6, 3).value.weak_eq mod.eval op.mul val(2, 1), val(3)
      
      it '2±1 * 3±1 = 6[-4+6]', ()->
        assert val(6, 4, 6).value.weak_eq mod.eval op.mul val(2, 1), val(3, 1)
      
      it '0±1 * 3±1 = 0±4', ()->
        assert val(0, 4).value.weak_eq mod.eval op.mul val(0, 1), val(3, 1)
      
      it '0±1 * 0±1 = 0±1', ()->
        assert val(0, 1).value.weak_eq mod.eval op.mul val(0, 1), val(0, 1)
